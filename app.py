...
with mysql.connector.connect(host=host, user=user, password=password, db=db) as conn:
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS survey(uname varchar(15),sid varchar(9), time int, url text, date timestamp default now() on update now(),foreign key(uname) references users(username))')
